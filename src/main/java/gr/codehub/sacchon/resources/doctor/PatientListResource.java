...
public class PatientListResource extends AuthResource {
    ...
    @Get("json")
    public List<PatientInfoRepresentation> getPatients(){
        ...
        setService(srv);
        return srv.getPatients((Doctor)getUser(),offset,limit).stream().map(PatientInfoRepresentation::new).collect(Collectors.toList());
    }
}
